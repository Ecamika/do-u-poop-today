...
function numToGreenWallCell(num: number, key: string, size: string) {
	...
	return (
		<div
			style={{ backgroundColor: rgb, width: size, height: size }}
			className="rounded"
			key={key}
		></div>
	)
...
export default function GreenWall({
	...
	const cells = wallGrids.map((v, i1) =>
		v.map((v, i2) => numToGreenWallCell(v, i1.toString() + i2.toString(), cellSize)),
	)

	const columns = cells.map((v, i) => (
		<div className="flex flex-col" style={{ gap: cellGap }} key={i}>
			{v}
		</div>
	))
